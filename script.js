...
function gameOver() {
  noLoop();
  background(0, 0, 0, 150); // خلفية شبه شفافة لتوضيح النص
  
  // نص Game Over
  textSize(64);
  fill(255, 0, 0); // لون أحمر
  textAlign(CENTER, CENTER);
  text("Game Over", width / 2, height / 2 - 50); // وضع النص أعلى قليلاً
  
  // زر Play Again
  playAgainButton.position(width / 2 - 100, height / 2 + 20); // مركز الزر
  playAgainButton.size(200, 50);
  playAgainButton.style('font-size', '24px');
  playAgainButton.style('background-color', '#4CAF50'); // لون أخضر
...
// كود الفوز (بنفس التنسيق)
if (gameWon) {
  background(0, 0, 0, 150);
  textSize(64);
  fill(0, 255, 0); // لون أخضر
  text("You Win!", width / 2, height / 2 - 50);
  playAgainButton.position(width / 2 - 100, height / 2 + 20);
  playAgainButton.size(200, 50);
  playAgainButton.style('font-size', '24px');
